...
- **Rebuilding from source invalidates the Accessibility grant.** The app is ad-hoc signed,
  so its signature changes with every build and macOS treats it as a new app. Re-add it in
  Accessibility after rebuilding. Installing to `/Applications` at least keeps the path
  stable.
